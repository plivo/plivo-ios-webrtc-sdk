...
void SipControllerCore::handleLog(std::string value) {
    ...
    if ((value.find("Failed to find connection") != std::string::npos) && (value.find("targetDomain="+m_domain) != std::string::npos)){
        m_connection_broken = true;
        
        handleLog("PLIVO DEBUG FLAG found connection termination from server | Reregister in ideal state");
        if (m_cert_id.empty() == false && m_cert_id != "NA" && m_cert_id != "" && m_app_id.empty() == false && m_app_id != "NA" && m_app_id != ""){
...
void SipControllerCore::registerUser(std::string username, std::string password, std::string token, std::string certid, std::string proxy, std::map<std::string,std::string> headers){
    ...
    if (certid != "NA" && token != "NA"){
        ...
        contactAddress = "<sip:" + username + "@" + m_public_ip_port + ";transport=tls;app_id=" + token + ">";
    }
    
    handleLog("registerUser: uri with (username/password/token/certid/proxy/headers): " + sipUri + " proxy " + m_proxyServer + " m_app id " + m_app_id + " m_cert_id " + m_cert_id + " contact " + contactAddress);
...
void SipControllerCore::setupTransport(std::string address, std::string contact_url){
    ...
    if (m_stack == nullptr){
        ...
        tls_key = m_stack->addTransport(TLS, 5061)->getKey();
...
void SipControllerCore::onSuccess(ClientRegistrationHandle, const SipMessage& response){
    ...
    if (m_connection_broken == true){
        m_connection_broken = false;
        networkChange();
    }
...
void SipControllerCore::onFailure(ClientRegistrationHandle, const SipMessage& response)
{
    ...
    if (errorHandler)
        errorHandler->handleError(SipSessionError, std::to_string(statusCode));
}
...
void SipControllerCore::onFailure(ClientInviteSessionHandle, const SipMessage& msg){
    ...
    callInfoHandler->handleCallInfo(processCallInfo(msg));
    handleLog("ClientInviteSessionHandle onFailure");
}
