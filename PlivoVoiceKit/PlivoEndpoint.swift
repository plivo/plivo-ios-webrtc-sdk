...
public class PlivoEndpoint: NSObject, PlivoLogDelegate {
    ...
    private var isInitialCallbackActive = false
...
//    private var networkMonitor: NWPathMonitor?
    ...
    private var logDelegate: PlivoLogDelegate?
    private var lastNetworkChange:Date?
    private var prevActiveInput: String = ""
    private var prevActiveOutput: String = ""
    private var areNotificationsRegistered: Bool = false
    ...
    private var lastNetworkType: NetworkStatus?
    private var isUserLoggedIn = false
    private var accessToken: JWT?
    private var jwtDelegate: JWTDelegate?
    ...
    private var callAndMediaMetrics: CallAndMediaMetrics?
    private var jwtErrorCodes: [String: Any] = [
        "10001":  "INVALID_ACCESS_TOKEN",
        "10002":  "INVALID_ACCESS_TOKEN_HEADER",
        "10003":  "INVALID_ACCESS_TOKEN_ISSUER",
        "10004":  "INVALID_ACCESS_TOKEN_SUBJECT",
        "10005":  "ACCESS_TOKEN_NOT_VALID_YET",
        "10006":  "ACCESS_TOKEN_EXPIRED",
        "10007":  "INVALID_ACCESS_TOKEN_SIGNATURE",
        "10008":  "INVALID_ACCESS_TOKEN_GRANTS",
        "10009":  "EXPIRATION_EXCEEDS_MAX_ALLOWED_TIME",
        "10010":  "MAX_ALLOWED_LOGIN_REACHED"
    ]

    private override init() {
        self.initOptions = [AnyHashable : Any]()
        self.initOptions["enableTracking"] = true
        PlivoQueue.shared.initFile(fileUrl: PlivoFile().plivoLogQueue()!)
...
//        self.initOptions = [AnyHashable : Any]()
        ...
        initOptions["idleAudioToggleNotification"] = false
        ...
        if let maxAverageBitrate = options["maxAverageBitrate"] as? Int {
            webrtcAdapter.webRTCClient.maxAverageBitrate = maxAverageBitrate
        }
        
        if let callAndMediaMetrics = options["enableQualityTracking"] as? CallAndMediaMetrics {
            self.callAndMediaMetrics = callAndMediaMetrics
            initOptions["enableQualityTracking"] = callAndMediaMetrics.rawValue
        } else if options["enableTracking"] == nil {
            self.callAndMediaMetrics = CallAndMediaMetrics.ALL
            initOptions["enableQualityTracking"] = CallAndMediaMetrics.ALL.rawValue
        }
        
        if let idleAudioToggleNotification = options["idleAudioToggleNotification"] as? Bool {
            initOptions["idleAudioToggleNotification"] = idleAudioToggleNotification
        }
    }
    ...
    @objc public convenience init(debug isDebug: Bool, _ options: [AnyHashable : Any]) {
        ...
        LogManager.shared.infoLogs(log: "Init | Plivo SDK initialized successfully with options:- \(self.initOptions)")
    ...
    @objc func applicationDidEnterBackground(_ notification:Notification){
        LogManager.shared.infoLogs(log: "App entered background")
        self.isApplicationinBackground = true
        
        if !self.isUserLoggedIn {
            LogManager.shared.log("Logged out returning from here", level: .debug)
            return
        }
        ...
        if let incoming = curIncomingCall{
            ...
            LogManager.shared.infoLogs(log: "Incoming | call Rejected")
        ...
        if let outgoing = curOutCall{
            ...
            LogManager.shared.infoLogs(log: "Outgoing | call Rejected")
    ...
    @objc func applicationWillEnterForeground(_ notification:Notification){
        LogManager.shared.infoLogs(log: "App entered foreground")
        ...
        if !isRegistered {
            if self.accessToken != nil && StatsConfig.access_token != nil && !StatsConfig.access_token!.isEmpty {
                var extraHeaders: [AnyHashable : Any] = [:]
                extraHeaders["X-Plivo-Jwt"] = StatsConfig.access_token
                sipClient?.registerUser(withUsername: StatsConfig.username, andPassword: "", andToken: StatsConfig.token ?? "NA", andCertificateId: StatsConfig.cert_id ?? "NA", andproxy: Constant.OUTBOUND_PROXY, andHeaders: extraHeaders, isJwt: true)
            } else if StatsConfig.cert_id?.isEmpty == false && StatsConfig.token != nil{
    ...
    private func NumberIsFraction(_ number: NSNumber?) -> Bool {
        let double = number?.doubleValue ?? 0.0
        let int = Double(number?.intValue ?? 0)
        let diff = double - int
        if diff > 0 {
            return true
        } else {
    ...
    @objc public func isUserRegistered() -> Bool {
            LogManager.shared.infoLogs(log: "isUserRegistered:\(String(self.isRegistered))")
    ...
    func sendLogs() {
        ...
        DispatchQueue.global(qos: .default).asyncAfter(deadline: .now() + .milliseconds(200)) {
            PlivoFile().flush(username: StatsConfig.username != nil ? StatsConfig.username! : "", jwtToken: StatsConfig.access_token != nil ? StatsConfig.access_token! : "")
        }
...
extension PlivoEndpoint{
    ...
    @objc private func reachabilityChanged(_ status: Notification) {
        ...
        if self.isInitialCallbackActive {
            LogManager.shared.infoLogs(log: "Network change | callback is called initially after attaching listener")
            DispatchQueue.global(qos: .default).async {
                Thread.sleep(forTimeInterval: 2.0)
                self.isInitialCallbackActive = false
            }
            return
        }
        ...
        if reachability.currentReachabilityStatus() != NotReachable && self.isUserLoggedIn {
            
            let privateIp: String = Utils().getIPAddress()
            let currentTime = Date()
            let diff = currentTime.timeIntervalSince(lastNetworkChange!)
            lastNetworkChange = currentTime
            LogManager.shared.infoLogs(log: "Network change | from type: \(String(describing: lastNetworkType)) to type: \(reachability.currentReachabilityStatus()) and from lastIp: \(lastNetworkIp ?? "nil") to currentIP: \(privateIp)")
            if (diff <= 5 && lastNetworkIp == privateIp) {
                LogManager.shared.log(Constant.FREQUENT_NETWORK_CHANGE_ERROR, level:.warning)
                return
            }
            
            lastNetworkIp = privateIp
            lastNetworkType = reachability.currentReachabilityStatus()
            LogManager.shared.log(Constant.RESET_SESSION_ON_NETWORK_CHANGE, level:.info)
            
            if self.accessToken != nil && StatsConfig.access_token != nil && !StatsConfig.access_token!.isEmpty && self.curOutCall == nil && self.curIncomingCall == nil {
                var extraHeaders: [AnyHashable : Any] = ["X-Plivo-Jwt": StatsConfig.access_token]
                LogManager.shared.infoLogs(log: "Network change | initiated")
                self.sipClient?.networkChange(extraHeaders)
            } else {
                if self.curOutCall != nil || self.curIncomingCall != nil {
                    LogManager.shared.infoLogs(log: "Network change | initiated during a call")
                } else {
                    LogManager.shared.infoLogs(log: "Network change | initiated")
                }
...
extension PlivoEndpoint{
    ...
    @objc public func login(_ username: String, andPassword password: String) {
        LogManager.shared.infoLogs(log: "Login | initiated with Endpoint with credentials: username: \(username), password: \(password)")
        if (username.isEmpty  || password.isEmpty) {
            LogManager.shared.log(Constant.EMPTY_LOGIN_1, level: .notice)
            LogManager.shared.infoLogs(log: "Login | failed: \(Constant.EMPTY_LOGIN_1)")
    ...
    /// Give users the ability to sign into plivo SIP server with timeout
    /// - Parameters:
    ///   - RegTimeout: registration timeout
    /// - Return:
    ///   - Bool: True if the timeout setup is succesfull, else false
    //Done
    @objc public func setRegTimeout(regTimeout: Int) -> Bool {
        if (regTimeout < 120 || regTimeout > 2592000) {
            LogManager.shared.log(Constant.REG_TIMEOUT_ERROR, level: .notice)
            LogManager.shared.infoLogs(log: "Login | Invalid Registration Timeout: \(regTimeout)")
        ...
        LogManager.shared.infoLogs(log: "Login | Timeout set for registration: \(regTimeout)")
        return true
    }
    ...
    @objc public func login(_ username: String, andPassword password: String, deviceToken token: Data?) {
        LogManager.shared.infoLogs(log: "Login | initiated with Endpoint with credentials: username: \(username), password: \(password), token: \(String(describing: token))")

        if (username.isEmpty  || password.isEmpty || token == nil) {
            LogManager.shared.log(Constant.EMPTY_LOGIN_2, level: .notice)
            LogManager.shared.infoLogs(log: "Login | failed: \(Constant.EMPTY_LOGIN_2)")
    ...
    @objc public func login(_ username: String, andPassword password: String, deviceToken token: Data?, certificateId: String) {
        LogManager.shared.infoLogs(log: "Login | initiated with Endpoint with credentials: username: \(username), password: \(password), token: \(String(describing: token)), certificateId: \(certificateId)")
        if (username.isEmpty  || password.isEmpty || token == nil || certificateId.isEmpty) {
            ...
            LogManager.shared.infoLogs(log: "Login | failed: \(Constant.EMPTY_LOGIN_3)")
    ...
    private func login(username: String, password: String, token: Data?, certificateId: String, _ pushInfo: [AnyHashable: Any]) {
        self.sipClient?.setRegisterTimeout(Constant.registrationTimeout as NSNumber)
        
        if pushInfo.isEmpty && isRegistered {
            LogManager.shared.infoLogs(log:Constant.ALREADY_REGISTERED)
            return
        }
        ...
        if Utils.isNetworkAvailable() == false{
            ...
            self.delegate?.onLoginFailedWithError?(error)
            LogManager.shared.log(Constant.DEBUG_LOGIN_FAILED_EVENT, level: .debug)
            LogManager.shared.infoLogs(log: "Login | failed: \(Constant.NO_INTERNET)")
            return
        }
        ...
        if(userName.count <= MAX_ENDPOINT_LENGTH && userName.count > 0) && userName.isAlphanumeric == true{
            ...
            rtpStats?.getCallStatsKey(self.accessToken)
            
            StatsConfig.token = deviceToken
            StatsConfig.cert_id = certificateId
            self.accessToken = nil
            StatsConfig.access_token = nil
            self.jwtDelegate = nil
            ...
            if pushInfo.isEmpty {
                ...
                self.relayVoipPushNotification(pushInfo)
            }
        }else{
            LogManager.shared.log(Constant.INVALID_SIP_USERNAME, level: .notice)
            LogManager.shared.infoLogs(log: "Login | Failed: \(Constant.INVALID_SIP_USERNAME)")
    ...
    @objc public func loginWithAccessToken(_ accessToken: String) {
        LogManager.shared.infoLogs(log: "Login | initiated with JWT with credentials: acessToken: \(accessToken)")
        
        if (accessToken.isEmpty) {
            LogManager.shared.log(Constant.JWT_EMPTY_LOGIN_1, level: .notice)
            LogManager.shared.infoLogs(log: "Login | failed: \(Constant.JWT_EMPTY_LOGIN_1)")
            guard let error = Utils.error(withMessage: Constant.JWT_EMPTY_LOGIN_1, code: 1112) else {
                LogManager.shared.log(Constant.FAILED_TO_CREATE_ERROR, level: .error)
                return
            }
            self.delegate?.onLoginFailed?()
        ...
        loginWithJWT(accessToken, deviceToken: nil, certificateId: "NA", [:])
    }
    
    @objc public func loginWithAccessToken(_ accessToken: String, deviceToken: Data?) {
        LogManager.shared.infoLogs(log: "Login | initiated with JWT with credentials: acessToken: \(accessToken), deviceToken: \(String(describing: deviceToken))")
        if (accessToken.isEmpty || deviceToken == nil) {
            LogManager.shared.log(Constant.JWT_EMPTY_LOGIN_2, level: .notice)
            LogManager.shared.infoLogs(log: "Login | failed: \(Constant.JWT_EMPTY_LOGIN_2)")
            guard let error = Utils.error(withMessage: Constant.JWT_EMPTY_LOGIN_2, code: 1112) else {
                LogManager.shared.log(Constant.FAILED_TO_CREATE_ERROR, level: .error)
                return
            }
            self.delegate?.onLoginFailed?()
        ...
        loginWithJWT(accessToken, deviceToken: deviceToken, certificateId: "NA", [:])
    }
    
    @objc public func loginWithAccessToken(_ accessToken: String, deviceToken: Data?, certificateId: String) {
        LogManager.shared.infoLogs(log: "Login | initiated with JWT with credentials: acessToken: \(accessToken), deviceToken: \(String(describing: deviceToken)), certificateId: \(certificateId)")
        if (accessToken.isEmpty || deviceToken == nil || certificateId.isEmpty) {
            LogManager.shared.log(Constant.JWT_EMPTY_LOGIN_3, level: .notice)
            LogManager.shared.infoLogs(log: "Login | failed: \(Constant.JWT_EMPTY_LOGIN_3)")
            guard let error = Utils.error(withMessage: Constant.JWT_EMPTY_LOGIN_3, code: 1112) else {
                LogManager.shared.log(Constant.FAILED_TO_CREATE_ERROR, level: .error)
                return
            }
            self.delegate?.onLoginFailed?()
        ...
        loginWithJWT(accessToken, deviceToken: deviceToken, certificateId: certificateId, [:])
    }
    
    
    @objc public func loginWithAccessTokenGenerator(jwtDelegate: JWTDelegate) {
        LogManager.shared.infoLogs(log: "Login | initiated with AccessTokenGenerator")
        self.jwtDelegate = jwtDelegate
        jwtDelegate.getAccessToken()
    }
    
    fileprivate func loginWithJWT(_ jwtToken: String, deviceToken deviceToken: Data?, certificateId: String, _ pushInfo: [AnyHashable: Any]) {
        
        self.sipClient?.setRegisterTimeout(Constant.registrationTimeout as NSNumber)

        if pushInfo.isEmpty && isRegistered{
            LogManager.shared.infoLogs(log: "Login | failed: \(Constant.ALREADY_REGISTERED)")
            guard let error = Utils.error(withMessage: Constant.ALREADY_REGISTERED, code: 291) else {
                LogManager.shared.log(Constant.FAILED_TO_CREATE_ERROR, level: .error)
                return
            }
        ...
        let parsedToken: [String : Any] = Utils.jwtDecode(jwtToken: jwtToken)
        if !parsedToken.isEmpty {
            if !validateJWT(token: parsedToken) {
                LogManager.shared.infoLogs(log: "Login | failed: \(Constant.INVALID_ACCESS_TOKEN)")
                guard let error = Utils.error(withMessage: Constant.INVALID_ACCESS_TOKEN, code: 10001) else {
                    LogManager.shared.log(Constant.FAILED_TO_CREATE_ERROR, level: .error)
                    return
                }
                self.delegate?.onLoginFailedWithError?(error)
                LogManager.shared.log(Constant.INVALID_ACCESS_TOKEN, level: .error)
                return
            }
            
            LogManager.shared.infoLogs(log: "Login | Parsed JWT with params: \(parsedToken)")
            if (self.accessToken!.sub == nil || self.accessToken!.sub!.isEmpty) {
                self.accessToken!.sub = self.getRandomSub()
            }
            
            if accessToken!.per == nil || (accessToken?.per != nil && accessToken?.per?.voice == nil) {

                let voice = Voice(incoming_allow: false, outgoing_allow: false)
                let per = Permission(voice: voice)
                accessToken!.per = per
            } else {
                if accessToken?.per?.voice?.outgoing_allow == nil {
                    accessToken?.per?.voice?.outgoing_allow = false
                }

                if accessToken?.per?.voice?.incoming_allow == nil {
                    accessToken?.per?.voice?.incoming_allow = false
                }
            }
            
            let username: String = self.accessToken!.sub! + "_" + self.accessToken!.iss
            
            StatsConfig.username = username
            StatsConfig.access_token = jwtToken
            StatsConfig.token = deviceToken?.hexString
            StatsConfig.cert_id = certificateId
            StatsConfig.password = nil
            
            rtpStats?.getCallStatsKey(self.accessToken)
            
            var extraHeaders: [AnyHashable : Any] = ["X-Plivo-Jwt": jwtToken]
            if pushInfo.isEmpty {
                sipClient?.registerUser(withUsername: username, andPassword: "", andToken: StatsConfig.token ?? "NA", andCertificateId: StatsConfig.cert_id ?? "NA", andproxy: Constant.OUTBOUND_PROXY, andHeaders: extraHeaders, isJwt: true)
            } else {
                self.relayVoipPushNotification(pushInfo)
            }
        } else {
    ...
    private func validateJWT(token jwtToken: [String: Any]) -> Bool {
        let jsonDecoder = JSONDecoder()
        guard let jsonData = try? JSONSerialization.data(withJSONObject: jwtToken), let accessToken: JWT = try? jsonDecoder.decode(JWT.self, from: jsonData) else {
            LogManager.shared.infoLogs(log: "Login | failed: Error while validating JWT \(Constant.INVALID_ACCESS_TOKEN)")
            guard let error = Utils.error(withMessage: Constant.INVALID_ACCESS_TOKEN, code: 10001) else {
                LogManager.shared.log(Constant.FAILED_TO_CREATE_ERROR, level: .error)
                return false
            }
    ...
    private func getRandomSub() -> String {
        return "puser" + String(random(digits: 10)) + "jt"
    }
    ...
    @objc public func registerToken(_ token: Data) {
        ...
        if let user = StatsConfig.username, let pass = StatsConfig.password, tokenBytes.isEmpty == false{
            ...
            sipClient?.registerUser(withUsername: user, andPassword: pass, andToken: tokenBytes, andCertificateId: "NA", andproxy: "NA", andHeaders: extraHeaders, isJwt: false)
        } else if accessToken != nil && StatsConfig.access_token != nil && !StatsConfig.access_token!.isEmpty{
            sipClient?.registerUser(withUsername: StatsConfig.username, andPassword: "NA", andToken: tokenBytes, andCertificateId: "NA", andproxy: "NA", andHeaders: extraHeaders, isJwt: true)
        } else{
            LogManager.shared.log(Constant.EMPTY_LOGIN_2, level: .notice)
    ...
    @objc public func loginForIncomingWithUsername(withUserName username: String, withPassword password: String, withDeviceToken deviceToken: Data?, withCertifateId certificateId: String?, withNotificationInfo pushInfo: [AnyHashable: Any]) {
        LogManager.shared.infoLogs(log: "Incoming | call registeration initiated with username:- \(username), password: \(password), deviceToken: \(deviceToken), certificateId: \(certificateId) and payload: \(pushInfo)")
        if (username.isEmpty  || password.isEmpty) {
            LogManager.shared.log(Constant.EMPTY_LOGIN_1, level: .notice)
            LogManager.shared.infoLogs(log: "Incoming | failed: \(Constant.EMPTY_LOGIN_1)")
            guard let error = Utils.error(withMessage: Constant.EMPTY_LOGIN_1, code: 1111) else {
                LogManager.shared.log(Constant.FAILED_TO_CREATE_ERROR, level: .error)
                return
            }
        ...
        self.login(username: username, password: password, token: deviceToken, certificateId: certificateId ?? "", pushInfo)
    }
    
    @objc public func loginForIncomingWithJWT(withAccessToken accessToken: String, withDeviceToken deviceToken: Data?, withCertificateId certificateId: String?, withNotificationInfo pushInfo: [AnyHashable: Any]) {
        LogManager.shared.infoLogs(log: "Incoming | call registeration initiated with accessToken:- \(accessToken), deviceToken: \(deviceToken), certificateId: \(certificateId) and payload: \(pushInfo)")
        if (accessToken.isEmpty) {
            LogManager.shared.log(Constant.JWT_EMPTY_LOGIN_1, level: .notice)
            LogManager.shared.infoLogs(log: "Incoming | failed: \(Constant.JWT_EMPTY_LOGIN_1)")
            guard let error = Utils.error(withMessage: Constant.JWT_EMPTY_LOGIN_1, code: 1112) else {
                LogManager.shared.log(Constant.FAILED_TO_CREATE_ERROR, level: .error)
                return
            }
        ...
        self.loginWithJWT(accessToken, deviceToken: deviceToken, certificateId: certificateId ?? "", pushInfo)
    }
    
    @available(*, deprecated, message: "'relayVoipPushNotification:pushinfo' is deprecated. Use `loginForIncomingWithUsername` or `loginForIncomingWithJWT` instead")
    /// Description
    ...
    @objc public func relayVoipPushNotification(_ pushinfo: [AnyHashable : Any]) {
        ...
        if state == .background {
            ...
            LogManager.shared.infoLogs(log: "Incoming | call received in background state")
            registerNotifications()
            self.isApplicationinBackground = true
        }
        ...
        if accessToken != nil && self.accessToken?.per?.voice?.incoming_allow == false {
            LogManager.shared.infoLogs(log: "Incoming | call permission not granted")
            guard let error = Utils.error(withMessage: Constant.INVALID_ACCESS_TOKEN_GRANTS, code: 10008) else {
                LogManager.shared.log(Constant.FAILED_TO_CREATE_ERROR, level: .error)
                return
            }
            LogManager.shared.log(Constant.INVALID_ACCESS_TOKEN_GRANTS, level: .warning)
            self.delegate?.onPermissionDenied(error)
            return
        }
        ...
        extraHeaders["X-Plivo-Registrar-Info"] = registrar
        extraHeaders["X-Plivo-Stir-Verification"] = stir
        
        if accessToken != nil && StatsConfig.access_token != nil && !StatsConfig.access_token!.isEmpty {
            extraHeaders["X-Plivo-Jwt"] = StatsConfig.access_token
        }
        
        if isOngoing == false{
            self.curIncomingCall = PlivoIncoming(sipAdapter: sipObj, webrtcAdapter: self.webrtcAdapter, feedbackManager: self.feedbackManager!, rtpStats: self.rtpStats!)
        ...
        if let user = StatsConfig.username, let pass = StatsConfig.password{
            sipClient?.registerUser(withUsername: user, andPassword: pass, andToken: StatsConfig.token ?? "NA", andCertificateId: StatsConfig.cert_id ?? "NA", andproxy: Constant.OUTBOUND_PROXY, andHeaders: extraHeaders, isJwt: false)
        } else if accessToken != nil && StatsConfig.access_token != nil && !StatsConfig.access_token!.isEmpty {
            sipClient?.registerUser(withUsername: StatsConfig.username ?? "NA", andPassword: "NA", andToken: StatsConfig.token ?? "NA", andCertificateId: StatsConfig.cert_id ?? "NA", andproxy: Constant.OUTBOUND_PROXY, andHeaders: extraHeaders, isJwt: true)
        }else{
            LogManager.shared.log(Constant.EMPTY_LOGIN_1, level: .emergency)
            LogManager.shared.infoLogs(log: "Incoming | call login failed: \(Constant.EMPTY_LOGIN_1)")
            guard let error = Utils.error(withMessage: Constant.EMPTY_LOGIN_1, code: 1111) else {
                LogManager.shared.log(Constant.FAILED_TO_CREATE_ERROR, level: .error)
                return
            }
...
//        LogManager.shared.log("Checking Incoming Invite in ringing state", level: .debug)
    ...
    @objc public func logout(){
        ...
        LogManager.shared.infoLogs(log: "Logout | Initiating logout")
        webrtcAdapter.hangupCall()
        sipClient?.hangupSession()
        if self.accessToken != nil && StatsConfig.access_token != nil && !StatsConfig.access_token!.isEmpty {
            var extraHeaders: [AnyHashable : Any] = [:]
            extraHeaders["X-Plivo-Jwt"] = StatsConfig.access_token
            sipClient?.unregisterUser(extraHeaders)
        } else {
...
extension PlivoEndpoint{
    ...
    @objc public func createOutgoingCall() -> PlivoOutgoing? {
        ...
        if accessToken != nil && self.accessToken?.per?.voice?.outgoing_allow == false {
            guard let error = Utils.error(withMessage: Constant.INVALID_ACCESS_TOKEN_GRANTS, code: 10008) else {
                LogManager.shared.log(Constant.FAILED_TO_CREATE_ERROR, level: .error)
                return nil
            }
            LogManager.shared.infoLogs(log: "Outgoing | call permission not granted")
            LogManager.shared.log(Constant.INVALID_ACCESS_TOKEN_GRANTS, level: .warning)
            self.delegate?.onPermissionDenied(error)
        ...
        let outGoingCall = PlivoOutgoing(sipAdapter: sipObj, webrtcAdapter: self.webrtcAdapter, feedbackManager: self.feedbackManager!, rtpStats: self.rtpStats!)
    ...
    @objc public func getLastCallUUID() -> String? {
        if StatsConfig.last_xcall_uuid == nil || StatsConfig.last_xcall_uuid == ""{
            return nil
        }
        return StatsConfig.last_xcall_uuid
    }
    ...
    fileprivate func incomingCallStates(_ otherReason: OtherReason?,_ terminateReason: TerminatedReason?, _ statusCode: Int32) {
        ...
        if otherReason != nil{
            ...
            if otherReason == .CallAccepted{
                ...
                LogManager.shared.infoLogs(log: "Incoming | call Answered")
...
//                PlivoIncoming *incoming = plivo_incoming_object(acc_id, call_id);
            ...
            if  (statusCode == 408){
                LogManager.shared.infoLogs(log: "Incoming | call Invalid")
...
//                PlivoIncoming *incoming = plivo_incoming_object(acc_id, call_id);
                ...
                LogManager.shared.infoLogs(log: "Incoming | call Hangup Remotely")
                self.logDelegate?.sendLogs()
...
//                    set_outgoing(call_id);
            ...
            if (180...183).contains(statusCode){
                if let out = self.curOutCall{
                    ...
                    outgoing.callId = callId ?? "NA"
                    LogManager.shared.infoLogs(log: "Outgoing | call Ringing")
                    self.delegate?.onOutgoingCallRinging?(outgoing)
                    if let callId = self.callId{
                        self.sendCallRingingStats(call_id: callId)
                    }else{
                        LogManager.shared.log("not sending call summary on call ringing | callid is nil | might be duplicate termination", level: .debug)
                    }
...
//                    set_outgoing(call_id);
                    ...
                    StatsConfig.call_confirmed_time = Utils.getCurrentTimeInMilliSeconds()
                    outgoing.callId = callId ?? "NA"
                    LogManager.shared.infoLogs(log: "Outgoing | call Answered")
        ...
        if terminateReason != nil{
            ...
            if  (404...408).contains(statusCode){
                if let out = self.curOutCall{
                    ...
                    outgoing.state = .Terminated
                    outgoing.callId = callId ?? "NA"
                    LogManager.shared.infoLogs(log: "Outgoing | call Invalid")
            ...
            if  statusCode == 200{
                if let out = self.curOutCall{
                    ...
                    updateLastCallDetails()
                    self.curOutCall?.stopSpeechRecognition()
                    LogManager.shared.infoLogs(log: "Outgoing | call Hangup Remotely")
                    self.logDelegate?.sendLogs()
    ...
    fileprivate func terminateCall(_ value: String?) {
        LogManager.shared.infoLogs(log: "Call | TerminateCall callId: \(self.callId ?? "NA"), reason: \(value ?? "NA")")
        ...
        if let callId = self.callId{
            self.sendCallSummaryStats(call_id: callId)
        }else{
        ...
        self.curIncomingCall?.state = .Terminated
        self.curOutCall?.state = .Terminated
        
        
        if let out = self.curOutCall{
            if value == "Remote Rejected" {
                LogManager.shared.infoLogs(log: "Outgoing | call Rejected remotely")
            } else if value == "Local Ignored" {
                LogManager.shared.infoLogs(log: "Outgoing | call Hangup locally")
            } else {
                LogManager.shared.infoLogs(log: "Outgoing | call Hangup remotely")
            }
            self.curOutCall?.stopSpeechRecognition()
            self.delegate?.onOutgoingCallHangup?(out)
        }
        
        if let inCall = self.curIncomingCall{
            if value == "Local Ignored" {
                LogManager.shared.infoLogs(log: "Incoming | call hangup locally")
            } else if value == "Local Rejected" {
                LogManager.shared.infoLogs(log: "Incoming | call rejected locally")
            } else {
                LogManager.shared.infoLogs(log: "Incoming | call hangup remotely")
            }
            curIncomingCall?.stopSpeechRecognition()
            self.delegate?.onIncomingCallHangup?(inCall)
        }
        if !self.isLogOutInitiated {
            self.logDelegate?.sendLogs()
        }
        updateLastCallDetails()
        self.webrtcAdapter.hangupCall()
        Thread.sleep(forTimeInterval: 0.05)
        self.curIncomingCall = nil
        self.curOutCall = nil
        ...
        self.ringTimer?.invalidate()
        self.ringTimer = nil
        StatsConfig.x_call_uuid = nil
        StatsConfig.call_uuid = nil
    
        if self.isApplicationinBackground == true{
    ...
    fileprivate func sipClientEventHandler() {
        ...
        sipClient?.registerCallHandler { [weak self] (callEvent, value) in
            ...
            if (callEvent == IncomingCall) {
                ...
                strongSelf.terminateCall(value)
        ...
        sipClient?.registerRegistrationHandler { [weak self] (registrationEvent, value) in
            ...
            if (registrationEvent == Registered) {
                ...
                if !strongSelf.areNotificationsRegistered {
                    strongSelf.registerNotifications()
                }
                LogManager.shared.infoLogs(log: "Login | User logged in successfully")
                strongSelf.logDelegate?.sendLogs()
                LogManager.shared.log(Constant.DEBUG_ONLOGIN_EVENT, level: .debug)
            } else if (registrationEvent == NotRegisteredJWT) {
                strongSelf.isRegistered = false
                strongSelf.isUserLoggedIn = false
                LogManager.shared.log(Constant.DEBUG_ONLOGIN_EVENT, level: .debug)
                LogManager.shared.log(" sip Callback in endpoint registerRegistrationHandler : JWT registration error ", level: .info)
                
                if strongSelf.accessToken != nil {
                    let errorCode: String = (value?.components(separatedBy: "@@")[0] ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
                    if let error = Utils.error(withMessage: strongSelf.jwtErrorCodes[errorCode] as? String, code: Int(errorCode)!){
                        LogManager.shared.log(strongSelf.jwtErrorCodes[errorCode] as! String, level: .debug)
                        strongSelf.delegate?.onLoginFailed?()
                        strongSelf.delegate?.onLoginFailedWithError?(error)
                        LogManager.shared.infoLogs(log: "Login | JWT Login failed with code: \(errorCode), error: \(error) and stacktrace: \(value?.components(separatedBy: "@@")[1] ?? "")")
                        LogManager.shared.log(strongSelf.jwtErrorCodes[errorCode] as! String, level: .debug)
                    }
                    
                    if strongSelf.curIncomingCall != nil || strongSelf.curOutCall != nil {
                        strongSelf.terminateCall(errorCode)
                    }
                    if let delegate = strongSelf.jwtDelegate {
                        delegate.getAccessToken()
                    } else {
                        strongSelf.isRegistered = false
                        strongSelf.isUserLoggedIn = false
                        LogManager.shared.log(Constant.DEBUG_ONLOGOUT_EVENT, level: .debug)
                        LogManager.shared.infoLogs(log: "Logout | Token Expired. Initiating Logout") // TODO: add this in the docs
                        strongSelf.logout()
                        strongSelf.delegate?.onLogout?()
                        if strongSelf.areNotificationsRegistered {
                            strongSelf.unregisterNotifications()
                        }
                ...
                strongSelf.isRegistered = false
                strongSelf.isUserLoggedIn = false
                LogManager.shared.infoLogs(log: "Logout | Logout successful")
                strongSelf.isLogOutInitiated = false
        ...
        sipClient?.sipSDPHandler { [weak self] (sdp) in
            ...
            if let sdpString = sdp , sdpString != ""{
                ...
                if strongSelf.isIncomingCall{
//                    LogManager.shared.infoLogs(log: "Incoming | call SDP Offer received: \(sdpString)")
                    LogManager.shared.infoLogs(log: "Incoming | call SDP Offer received")
                    strongSelf.webrtcAdapter.setRemote(offerSDP: sdpString)
                    strongSelf.on_incoming_call(callInfo: strongSelf.callData)
                }else{
//                    LogManager.shared.infoLogs(log: "Outgoing | call SDP Answer received: \(sdpString)")
                    LogManager.shared.infoLogs(log: "Outgoing | call SDP Answer received")
        ...
        sipClient?.registerErrorHandler { [weak self] (errorType, value) in
            ...
            if let error = Utils.error(withMessage: "Error while registering status code: \(Int(value ?? "0") ?? 0)", code: Int(value ?? "0") ?? 0){
                ...
                LogManager.shared.infoLogs(log: "Login | Failed: \(value)" ?? "Registration failed reason: unknown")
                strongSelf.delegate?.onLoginFailed?()
                strongSelf.delegate?.onLoginFailedWithError?(error)
                LogManager.shared.log(Constant.DEBUG_LOGIN_FAILED_EVENT, level: .debug)
            }
...
//        self.webrtcAdapter.createAnswer()
    ...
    private func plivo_incoming_object_with_extra_headers(call:PlivoIncoming?, callInfo:String?)->PlivoIncoming{
        ...
        if call == nil {
            ...
            if self.sipClient != nil {
                incomingCall = PlivoIncoming(sipAdapter: sipClient!, webrtcAdapter: self.webrtcAdapter, feedbackManager: self.feedbackManager!, rtpStats: self.rtpStats!)
            }else{
                LogManager.shared.log("SipClient object is nil", level: .error)
            }
        }
...
//    }
    ...
    @objc public func resetEndpoint(){
        LogManager.shared.infoLogs(log: "ResetEndpoint | Resetting Endpoint and Transport stack")
        self.logDelegate?.sendLogs()
...
extension PlivoEndpoint:WebrtcAdapterIncomingDelegate{
    ...
    func didConnectCall() {
        ...
        if(self.curIncomingCall != nil) {
            self.delegate?.onIncomingCallConnected(self.curIncomingCall!)
        }
    }
...
extension PlivoEndpoint{
    ...
    @objc public func submitCallQualityFeedback(_ callUUID: String?, _ startRating: Int, _ issues: [AnyObject], _ notes: String, _ sendConsoleLog: Bool) {
        ...
        if !isRegistered{
            guard let error = Utils.error(withMessage: Constant.USER_NOT_REGISTERED, code: 291) else {
                LogManager.shared.log(Constant.FAILED_TO_CREATE_ERROR, level: .error)
                return
            }
            LogManager.shared.log(Constant.USER_NOT_REGISTERED, level: .warning)
            self.delegate?.onLoginFailedWithError(error)
            LogManager.shared.infoLogs(log: "SubmitCallQualityFeedback | failed: \(Constant.USER_NOT_REGISTERED)")
            return
        }
        
        guard let feedbackManager = self.feedbackManager else {
            LogManager.shared.log("Feedback manager object not found | Returning from code block", level: .error)
            return
        }
        
        var issueStringList = issues.map({$0 as? String ?? "NAC"}).map({$0.uppercased()})
        ...
        feedbackManager.sendQualityFeedbackStats(issueStringList, callUUID, startRating, notes)
        
        if sendConsoleLog {
            var param: [String: String] = [:]
            var isAccessTokenLogin = false
            if self.accessToken != nil && StatsConfig.access_token != nil && !StatsConfig.access_token!.isEmpty {
                isAccessTokenLogin = true
                param["jwt"] = StatsConfig.access_token
                param["call_uuid"] = callUUID ?? "NA"
            } else {
                param["username"] = StatsConfig.username ?? ""
                param["password"] = StatsConfig.password ?? ""
                param["calluuid"] = callUUID ?? "NA"
                param["domain"] = Environment.sipDomain
            }
            
            if JSONSerialization.isValidJSONObject(param) {
                
                HTTPClient.shared.postRequest(url: isAccessTokenLogin ? Constant.S3BUCKET_API_URL_JWT : Constant.S3BUCKET_API_URL, params: param) { response in
                    switch response{
                    case let .success(value):
                            ...
                            if let json = value as? [String: Any], let s3UrlString = json["data"] as? String{
                                ...
                                HTTPClient.shared.putRequest(urlString: s3UrlString, params: putData) { (response) in
                                    ...
                                    case let .failure(error):
                                        ...
                                        LogManager.shared.infoLogs(log: "SubmitCallQualityFeedback | failed with params callUUID: \(String(describing: callUUID)), starRating: \(startRating), issues: \(issues), notes: \(notes), sendConsoleLogs: \(sendConsoleLog) and reason: \(error)")
                                        break
                                    }
                                }
                            }
                        ...
                        if let json = value as? [String: Any], let s3UrlString = json["data"]{
                            ...
                            LogManager.shared.infoLogs(log: "SubmitCallQualityFeedback | failed with params callUUID: \(String(describing: callUUID)), starRating: \(startRating), issues: \(issues), notes: \(notes), sendConsoleLogs: \(sendConsoleLog) and reason: \(Constant.DEBUG_FEEDBACKALIDATION_FAUILER_EVENT)")
...
extension PlivoEndpoint: RTPStatsDelegate{
    ...
    func emitMediaMetrics(_ group: String, _ level: String, _ type: String, _ value: Double, _ active: Bool, _ desc: String, _ stream: String) {
        ...
        if (self.callAndMediaMetrics != nil &&  self.callAndMediaMetrics != CallAndMediaMetrics.REMOTE_ONLY && self.callAndMediaMetrics != CallAndMediaMetrics.NONE)
            || (self.callAndMediaMetrics == nil && self.initOptions["enableTracking"] != nil && self.initOptions["enableTracking"] as! Bool == true) {
    ...
    func speakOnMute() {
        self.delegate?.speakingOnMute()
    }
    
    private func updateLastCallDetails(){
        StatsConfig.last_call_uuid = StatsConfig.call_uuid
        StatsConfig.last_xcall_uuid = StatsConfig.x_call_uuid
        StatsConfig.last_stats_key = StatsConfig.stats_key
    }
    ...
    func sendCallRingingStats(call_id: String){
        
        if (self.callAndMediaMetrics != nil && (self.callAndMediaMetrics == CallAndMediaMetrics.NONE || self.callAndMediaMetrics == CallAndMediaMetrics.LOCAL_ONLY))
            || (StatsConfig.stats_key == nil)
            || (self.callAndMediaMetrics == nil && self.initOptions["enableTracking"] != nil && self.initOptions["enableTracking"] as! Bool == false) {
            LogManager.shared.log("sendCallRingingStats() | Not sending because callAndMediaMetrics == NONE || enableTracking == false || Stats Key is nil returing from here", level:.error)
            return
            
        }
        
        LogManager.shared.log("sendCallRingingStats() Processing with callid : \(call_id)", level:.debug)
        
        StatsConfig.call_uuid = call_id;
        
        guard let rtpStat = rtpStats else { LogManager.shared.log("sendCallRingingStats()  RTPStats object is nil", level:.error); return }
        ...
        statsDict["msg"] = Constant.CALL_RINGING_MSG;
        self.feedbackManager?.sendStats(statsDict: &statsDict, type: Constant.CALL_RINGING_MSG)
    }
    ...
    func sendCallSummaryStats(call_id: String){
        ...
        if (self.callAndMediaMetrics != nil && (self.callAndMediaMetrics == CallAndMediaMetrics.NONE || self.callAndMediaMetrics == CallAndMediaMetrics.LOCAL_ONLY))
            || (StatsConfig.stats_key == nil)
            || (self.callAndMediaMetrics == nil && self.initOptions["enableTracking"] != nil && self.initOptions["enableTracking"] as! Bool == false) {
            LogManager.shared.log("sendCallAnsweredStats() | Not sending because callAndMediaMetrics == NONE || enableTracking == false || Stats Key is nil returing from here", level:.error)
            return
        ...
        statsDict["signalling"] = signalDict
        statsDict["setupOptions"] = initOptions;
        statsDict["msg"] = Constant.CALL_SUMMARY_MSG;
    ...
    func sendCallAnsweredStats(call_id: String, type: String) {
        ...
        if (self.callAndMediaMetrics != nil && self.callAndMediaMetrics == CallAndMediaMetrics.NONE )
            || (StatsConfig.stats_key == nil)
            || (self.callAndMediaMetrics == nil && initOptions["enableTracking"] != nil && initOptions["enableTracking"] as! Bool == false) {
            LogManager.shared.log("sendCallAnsweredStats() | Not sending because callAndMediaMetrics == NONE || enableTracking == false || Stats Key is nil returing from here", level:.error)
            return
        ...
        statsDict["setupOptions"] = initOptions
        statsDict["msg"] = Constant.CALL_ANSWERED_MSG;
        
        if (self.callAndMediaMetrics != nil && self.callAndMediaMetrics != CallAndMediaMetrics.NONE && self.callAndMediaMetrics != CallAndMediaMetrics.LOCAL_ONLY)
            || (self.callAndMediaMetrics == nil && initOptions["enableTracking"] != nil && initOptions["enableTracking"] as! Bool == true) {
        ...
        if StatsConfig.rtp_enabled == true {
            ...
            if (self.callAndMediaMetrics != nil && self.callAndMediaMetrics  != CallAndMediaMetrics.NONE)
                || (initOptions["enableTracking"] != nil && ((initOptions["enableTracking"] != nil) == true)) {
    ...
    @objc func isCallOnMute()->Bool {
        return (self.curIncomingCall != nil && self.curIncomingCall?.isMute as! Bool == true) || (self.curOutCall != nil && self.curOutCall?.isMute as! Bool == true)
    }
    ...
    @objc func sendCallIntervalStats(_ timer: Timer) {
        ...
        self.webrtcAdapter.getStats { [self] (statsString, dumpString) in
            ...
            if statsString.isEmpty{
                ...
                if let stats = statsString.convertToDictionary(){
                    statsDict["local"] = rtpStat.getLocalStats(stats, isCallOnMute: isCallOnMute())
                ...
                statsDict["msg"] = Constant.CALL_STATS_MSG;
...
@objc public enum CallAndMediaMetrics: Int, RawRepresentable {
    case ALL
    case REMOTE_ONLY
    case LOCAL_ONLY
    case NONE
...
//MARK
//extension for handling change in input/output devices and change in network
extension PlivoEndpoint {
    
    func registerNetworkChangeListener() -> Void {
        if hostReachability == nil && internetReachability == nil {
            ...
            self.isInitialCallbackActive = true
            LogManager.shared.infoLogs(log: "Network change | Network change listener registered successfully")
        }
        
    }
    
    func unregisterNetworkChangeListener() -> Void {
        NotificationCenter.default.removeObserver(self, name: NSNotification.Name.reachabilityChanged, object: nil)
        self.hostReachability = nil
        self.internetReachability = nil
        self.isInitialCallbackActive = false
        LogManager.shared.infoLogs(log: "Network change | Network change listener un-registered successfully")
    }
    ...
    func registerNotifications() -> Void {
        ...
        do {
            try sessionInstance.setCategory(.playAndRecord, mode: .voiceChat)
        } catch {
            print("Error getting audio devices: \(error.localizedDescription)")
        }
        ...
        self.prevActiveInput = sessionInstance.currentRoute.inputs.first?.uid ?? ""
        
        self.prevActiveOutput = sessionInstance.currentRoute.outputs.first?.uid ?? ""
        let inputType = sessionInstance.currentRoute.inputs.first?.portType.rawValue ?? ""
        let outputType = sessionInstance.currentRoute.outputs.first?.portType.rawValue ?? ""
        self.areNotificationsRegistered = true
        LogManager.shared.infoLogs(log: "Audio Toggle listener registered successfully")
        LogManager.shared.infoLogs(log: "Audio device set initially with inputType: \(inputType), inputName: \(self.prevActiveInput), outputType: \(outputType) and outputName: \(self.prevActiveOutput)")
    ...
    func unregisterNotifications() -> Void {
        ...
        self.areNotificationsRegistered = false
        LogManager.shared.infoLogs(log: "Audio Toggle listener unregistered successfully")
    }
    ...
    @objc func handleAudioRouteChange(_ notification: Notification) {
        ...
        if let reason = AVAudioSession.RouteChangeReason(rawValue: reasonValue) {
            ...
            case .override:
                deviceChange = "override"
            case .categoryChange:
                deviceChange = "categoryChange"
            case .unknown:
                break
            case .wakeFromSleep:
                break
        ...
        let currentActiveInput = currentRoute.inputs.first?.uid ?? ""
        let currentActiveOutput = currentRoute.outputs.first?.uid ?? ""
        
        //To handle the input device set to empty string after the callkit call is disconnected
        if !deviceChange.isEmpty && (currentActiveInput.isEmpty || currentActiveOutput.isEmpty) {
            if let idleDeviceToggleFlag =  self.initOptions["idleAudioToggleNotification"] as? Bool, idleDeviceToggleFlag == true {
                do {
                    try AVAudioSession.sharedInstance().setCategory(.playAndRecord, mode: .voiceChat)
                    //setting this true will stop any other app playing any audio as soon as the current app is active
                    try AVAudioSession.sharedInstance().setActive(true)
                } catch {
                    print("Error getting audio devices: \(error.localizedDescription)")
                }
            } else {
                let prevInputType = routeDescription?.inputs.first?.portType.rawValue ?? ""
                let currentInputType = currentRoute.inputs.first?.portType.rawValue ?? ""
                LogManager.shared.infoLogs(log: "Audio device toggled from type: \(prevInputType), name: \(prevActiveInput) TO type: \(currentInputType), name: \(currentActiveInput) but could not be sent to mediametrics event since idleAudioToggleNotification is false")
            }
        }
        
        if !deviceChange.isEmpty && !currentActiveInput.isEmpty && !currentActiveOutput.isEmpty && (self.prevActiveInput != currentActiveInput || self.prevActiveOutput != currentActiveOutput) {
            if (self.curIncomingCall != nil && self.curIncomingCall?.state == .Ongoing) || (self.curOutCall != nil && self.curOutCall?.state == .Ongoing) && self.rtpStats != nil && statsTimer != nil {
                // send event to call-insights
                let audioStats = self.rtpStats!.getAudioStats(totalDevices: AVAudioSession.sharedInstance().availableInputs ?? [], currentRoute: AVAudioSession.sharedInstance().currentRoute)
                
                var statsDict = self.rtpStats!.getInitialStats(isCallAnswered: false)
                statsDict["msg"] = Constant.AUDIO_DEVICE_TOGGLE
                statsDict["domain"] = Constant.DOMAIN
                statsDict["version"] = Constant.PLIVO_ENDPOINT_VER
                statsDict["deviceInfo"] = audioStats
                statsDict["source"] = Constant.SDK_NAME
                statsDict["timestampt"] = NSDate().timeIntervalSince1970
                self.feedbackManager?.sendStats(statsDict: &statsDict, type: Constant.AUDIO_DEVICE_TOGGLE)
            } else {
                let prevInputType = routeDescription?.inputs.first?.portType.rawValue ?? ""
                let prevOutputType = routeDescription?.outputs.first?.portType.rawValue ?? ""
                let currentInputType = currentRoute.inputs.first?.portType.rawValue ?? ""
                let currentOutputType = currentRoute.outputs.first?.portType.rawValue ?? ""
                LogManager.shared.infoLogs(log: "Audio device toggled from prevInputType: \(prevInputType), prevInputName: \(prevActiveInput), prevOutputType: \(prevOutputType), prevOutputName: \(prevActiveOutput) TO currentInputType: \(currentInputType), currentInputName: \(currentActiveInput) currentOutputType: \(currentOutputType) and currentOutputname: \(currentActiveOutput)")
            }

            if deviceChange == "removed" {
                if let prevRout = routeDescription {
                    self.delegate?.audioDeviceChange?(deviceChange: deviceChange, deviceInfo: prevRout)
                }
            } else {
                self.delegate?.audioDeviceChange?(deviceChange: deviceChange, deviceInfo: currentRoute)
            }
            prevActiveInput = currentActiveInput
            prevActiveOutput = currentActiveOutput
