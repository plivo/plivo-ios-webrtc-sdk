...
//--------------------------------------------------------------------------
// MARK: - PlivoCrashDelegate
//--------------------------------------------------------------------------
public protocol PlivoCrashDelegate: NSObjectProtocol {
    func plivoCrashDidCatchCrash(with model:CrashModel)
}

//--------------------------------------------------------------------------
// MARK: - WeakPlivoCrashDelegate
//--------------------------------------------------------------------------
//class WeakPlivoCrashDelegate: NSObject {
////    weak var delegate: PlivoCrashDelegate?
//
//    init(delegate: PlivoCrashDelegate) {
//        super.init()
...
// MARK: - PlivoCrash
//--------------------------------------------------------------------------
public class PlivoCrash: NSObject {
...
//        if contains == false {
//            let week = WeakPlivoCrashDelegate(delegate: delegate)
...
//    open class func remove(delegate:PlivoCrashDelegate) {
//        self.delegates = self.delegates.filter {
...
//    }
    ...
    private class func open() {
        ...
        PlivoCrash.isOpen = true
        
        app_old_exceptionHandler = NSGetUncaughtExceptionHandler()
        NSSetUncaughtExceptionHandler(PlivoCrash.RecieveException)
    ...
    private class func close() {
        ...
        PlivoCrash.isOpen = false
        NSSetUncaughtExceptionHandler(app_old_exceptionHandler)
    }
    
    private class func setCrashSignalHandler(){
        signal(SIGABRT, PlivoCrash.RecieveSignal)
        signal(SIGILL, PlivoCrash.RecieveSignal)
        signal(SIGSEGV, PlivoCrash.RecieveSignal)
        signal(SIGFPE, PlivoCrash.RecieveSignal)
        signal(SIGBUS, PlivoCrash.RecieveSignal)
        signal(SIGPIPE, PlivoCrash.RecieveSignal)
        //http://stackoverflow.com/questions/36325140/how-to-catch-a-swift-crash-and-do-some-logging
        signal(SIGTRAP, PlivoCrash.RecieveSignal)
        signal(SIGUSR1, PlivoCrash.RecieveSignal)
    }
    ...
    private static let RecieveException: @convention(c) (NSException) -> Swift.Void = {
        ...
        guard PlivoCrash.isOpen == true else {
            return
        }
        ...
        let appinfo = PlivoCrash.appInfo()
        ...
        NSLog("received crash exception with name \(callStack)")
        
        let logValue = "CRASH Exception: type: \(CrashModelType.exception) name: \(name.rawValue) reason: \(reason) appinfo: \(appinfo) callstack: \(callStack)"
        PlivoQueue.shared.insert(logValue)
    }
    ...
    private static let RecieveSignal : @convention(c) (Int32) -> Void = {
        ...
        let reason = "Signal \(PlivoCrash.name(of: signal))(\(signal)) was raised.\n"
        let appinfo = PlivoCrash.appInfo()
        
        let model = CrashModel(type:CrashModelType.signal,
                               name:PlivoCrash.name(of: signal),
                               reason:reason,
                               appinfo:appinfo,
                               callStack:callStack)
        
        NSLog("received crash signal with name \(callStack)")
        let logValue = "CRASH: type: \(CrashModelType.signal) name: \(PlivoCrash.name(of: signal)) reason: \(reason) appinfo: \(appinfo) callstack: \(callStack)"
        PlivoQueue.shared.insert(logValue)
    ...
    //--------------------------------------------------------------------------
//    fileprivate static var delegates = [WeakPlivoCrashDelegate]()
}
