...
void SipControllerCoreWrapper::registerUser(std::string username, std::string password, std::string token, std::string certid, std::string proxy, std::map<std::string,std::string> headers)
{
    m_sipControllerCore->registerUser(username, password, token, certid, proxy, headers);
}
...
void SipControllerCoreWrapper::networkChange(){
    m_sipControllerCore->networkChange();
}
...
- (id)init {
    if (self = [super init]) {
        ...
        std::string proxyString([[Environment sipDomain] UTF8String]);
...
- (void)registerUserWithUsername:(NSString *)username andPassword:(NSString *)password andToken:(NSString *)token andCertificateId:(NSString *)certid andproxy:(NSString *)proxy andHeaders:(NSDictionary *)headers{
    ...
    _sipControllerCoreWrapper->registerUser(usernameString, passwordString, deviceToken, certificateId, proxyAddress, h);
}
...
- (void)networkChange{
    _sipControllerCoreWrapper->networkChange();
}
