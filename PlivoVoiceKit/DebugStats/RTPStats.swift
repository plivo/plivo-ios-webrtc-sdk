...
class RtpStats: NSObject {
    ...
    func startSpeechRecognition(webrtcAdapter:WebrtcAdapter) {
        LogManager.shared.infoLogs(log: "Call | Speech recognition start")
        if let speechTimer = speechTimer {
            LogManager.shared.infoLogs(log: "Call | Speech recognition was active before muting the call")
            speechTimer.invalidate()
            self.speechTimer = nil
        }
        speechTimer = Timer.scheduledTimer(withTimeInterval: Constant.SPEECH_TIMER, repeats: true) { timer in
            webrtcAdapter.getStats {(statsString, dumpString) in self
                self.processStats(statsString: statsString, rtpStat: self)
            }
        }
    }
    
    func processStats(statsString: String, rtpStat: RtpStats) {
        if statsString.isEmpty{
            LogManager.shared.infoLogs(log: "Call | statString empty")
        } else {
            if let stats = statsString.convertToDictionary(){
                if let mediasource = stats["media-source"] as? [String:Any]{
                    let audioLevel = rtpStat.getAudioLevel(mediasource["audioLevel"] as? Double ?? 0.0)
                    if(audioLevel > Constant.SPEECH_RECOGNITION_THRESHOLD){
                        LogManager.shared.infoLogs(log: "Call | User trying to speak on mute")
                        self.delegate?.speakOnMute()
                        self.stopSpeechRecognition()
                    }
                }
            } else {
                LogManager.shared.infoLogs(log: "Call | RTCStats dictionary is not valid")
            }
        }
    }
    ...
    func stopSpeechRecognition(){
        if let speechTimer = speechTimer {
            LogManager.shared.infoLogs(log: "Call | Speech recognition stopped")
            speechTimer.invalidate()
            self.speechTimer = nil
        }
    }
...
//                LogManager.shared.log( "Audio mute detected for \(type): \(metricArr.componentsJoined(by: ","))", level: .debug)
    ...
    public func getAudioLevel(_ audioLevelAmplitude: Double) -> Double {
    ...
    func getInitialStats(isCallAnswered: Bool) -> [String : Any] {
        ...
        if sdkVersion.count == 3 {
            dict["sdkVersionMajor"] = sdkVersion[0]
            dict["sdkVersionMinor"] = sdkVersion[1]
            dict["sdkVersionPatch"] = sdkVersion[2]
        } else {
            dict["sdkVersionMajor"] = NSNull()
            dict["sdkVersionMinor"] = NSNull()
            dict["sdkVersionPatch"] = NSNull()
        }
        
        return dict
    }

    func getLocalStats(_ streamStats: [String : Any], isCallOnMute: Bool ) -> [String : Any] {
    ...
    func getCallStatsKey(_ accessToken: JWT?) {
        LogManager.shared.log(" getCallStatsKey() | Getting call stats key", level: .debug)
        
        let jwtToken = StatsConfig.access_token ?? ""
        var userDictionary: [String: String] = [:]
        if (!jwtToken.isEmpty) {
            userDictionary = ["jwt": jwtToken]
            
            if (accessToken != nil && (accessToken!.sub != nil) && (!(accessToken!.sub!.isEmpty)) && (accessToken!.sub!.hasPrefix("puser"))) {
                userDictionary["from"] = StatsConfig.username
            }
        } else {
            userDictionary = [
        ...
        if JSONSerialization.isValidJSONObject(userDictionary) {
            HTTPClient.shared.postRequest(url: jwtToken.isEmpty ? Constant.STATS_API_URL : Constant.STATS_API_URL_ACCESS_TOKEN, params: userDictionary) { (response) in
                switch response{
                case let .success(value):
                    if let json = value as? [String: Any]{
                        StatsConfig.stats_key = json["data"] as? String ?? ""
                        LogManager.shared.infoLogs(log: "Login | Call Stats key generated: \(StatsConfig.stats_key)")
                        ...
                        LogManager.shared.infoLogs(log: "Login | Call Stats key generation failed: Call stats is disabled")
                ...
                case let .failure(error):
                    ...
                    LogManager.shared.infoLogs(log: "Login | Call Stats key generation failed")
                    StatsConfig.stats_key = nil
                    StatsConfig.rtp_enabled = false
                    break
                }
    ...
    func getAudioStats(totalDevices: [AVAudioSessionPortDescription], currentRoute: AVAudioSessionRouteDescription) -> [String: Any] {
        ...
        for device in totalDevices {
            deviceIds.append(device.uid)
            deviceLabels.append(device.portName)
        }
        
        stats["activeInputAudioDevice"] = currentRoute.inputs.first?.portName
        stats["activeOutputAudioDevice"] = currentRoute.outputs.first?.portName
        stats["audioInputGroupIds"] = deviceIds.joined(separator: ",")
        stats["audioInputLables"] = deviceLabels.joined(separator: ",")
        for (index, device) in deviceIds.enumerated() {
            if (deviceIds[index] == "Built-In Microphone") {
                deviceIds.remove(at: index)
                deviceIds.insert("Built-In Receiver", at: index)
            }
        }
        
        for (index, device) in deviceLabels.enumerated() {
            if (deviceLabels[index] == "iPhone Microphone") {
                deviceLabels.remove(at: index)
                deviceLabels.insert("Receiver", at: index)
            }
        }
        stats["audioOutputGroupIds"] = deviceIds.joined(separator: ",")
        stats["audioOutputLables"] = deviceLabels.joined(separator: ",")
        stats["noOfAudioInput"] = totalDevices.count
        stats["noOfAudioOutput"] = totalDevices.count
